    ...
    if (config.qualifiedType === 'top2') {
      ...
      if (bestSecondsNeeded > 0) {
        ...
        bestSeconds.forEach(standing => {
          qualifiedTeams.push({
            group: standing.group_label || 'Sin grupo',
...
async function updateNextMatchRelations(
  ...
  if (totalQualified === 4) {
    ...
    if (semiFinals.length === 2 && finals.length === 1) {
      ...
      for (const semiId of semiFinals) {
        const { error } = await supabase
          .from('matches')
          .update({ next_match_id: finalId })
          .eq('id', semiId)
        
        if (error) {
          console.error('❌ Error al actualizar semifinal:', error)
        }
      }
    ...
    if (quarters.length === 4 && semis.length === 2 && finals.length === 1) {
      ...
      // Cuartos 1-2 → Semi 1
      if (quarters[0] && semis[0]) {
        await supabase.from('matches').update({ next_match_id: semis[0] }).eq('id', quarters[0])
      }
      if (quarters[1] && semis[0]) {
        await supabase.from('matches').update({ next_match_id: semis[0] }).eq('id', quarters[1])
      }
      // Cuartos 3-4 → Semi 2
      if (quarters[2] && semis[1]) {
        await supabase.from('matches').update({ next_match_id: semis[1] }).eq('id', quarters[2])
      }
      if (quarters[3] && semis[1]) {
        await supabase.from('matches').update({ next_match_id: semis[1] }).eq('id', quarters[3])
      }
      // Semis → Final
      if (semis[0] && finalId) {
        await supabase.from('matches').update({ next_match_id: finalId }).eq('id', semis[0])
      }
      if (semis[1] && finalId) {
        await supabase.from('matches').update({ next_match_id: finalId }).eq('id', semis[1])
      }
